-- X-Store 测试数据初始化脚本
-- 使用方法: mysql -h 127.0.0.1 -u root -proot x_store < init_test_data.sql

-- 0. 插入管理员账号 (密码: admin123)
INSERT IGNORE INTO users (username, email, password, nickname, role, status, created_at, updated_at) VALUES
('admin', 'admin@x-store.com', '$2a$10$l4NijSUwgSfF3rMRrzl17uEMhtRV1ssOXD1hGL.zSsgAyJQIsT1mW', '超级管理员', 'admin', 1, NOW(), NOW());
...
(1, 'ChatGPT Plus 独享账号', '', 120.00, 150.00, 100, 328, 'auto', '["自动发货","独享资源","7天质保"]', 100, 1, 1, 1, NOW(), NOW()),
(1, 'Claude Pro 会员账号', '', 98.00, 128.00, 50, 156, 'auto', '["自动发货","独享资源"]', 90, 1, 1, 0, NOW(), NOW()),
(2, 'Netflix 高级会员 1个月', '', 35.00, 45.00, 200, 892, 'auto', '["自动发货","4K画质"]', 80, 1, 0, 1, NOW(), NOW()),
(2, 'Spotify Premium 家庭版', '', 28.00, 35.00, 150, 445, 'auto', '["自动发货","6人共享"]', 70, 1, 0, 0, NOW(), NOW()),
(3, 'Steam 充值卡 100元', '', 95.00, 100.00, 300, 1203, 'auto', '["自动发货","秒到账"]', 60, 1, 0, 1, NOW(), NOW()),
(4, 'Apple Gift Card $50', '', 320.00, 350.00, 80, 234, 'auto', '["自动发货","全球通用"]', 50, 1, 0, 0, NOW(), NOW());

-- 3. 插入商品详情
INSERT INTO product_details (product_id, description, notice, created_at, updated_at) VALUES
...
('模拟支付（测试）', 'mock', 'mockpay', 'redirect', '{"secret":"mock-secret-key-2026","notify_url":"http://localhost:8080/api/webhooks/payment/1","return_url":"http://localhost:3000/order-success","auto_notify":true}', 0.00, 0.00, 1, 100, NOW(), NOW());
...
SELECT '=== 商品数据 ===' as info;
SELECT id, title, price, stock, sales, delivery_type, status FROM products;
